...
(function($) {
	$.fn.avgrund = function(options) {
		var defaults = {
			...
			onLoad: function() {},
			onUnload: function() {},
			template: '<p>This is besth popin content!</p>'
		};
		...
		return this.each(function() {
			...
			if($('.avgrund-overlay').length != 1)
			{
				body.append('<div class="avgrund-overlay ' + options.overlayClass + '"></div>');				
				body.append('<div id="besth" class="avgrund-popin ' + options.holderClass + '">' + template + '</div>');
			}
			...
			function deactivate() {
				...
				//$('div').remove('.avgrund-overlay ' + options.overlayClass);
				//$('div').remove('.avgrund-popin ' + options.holderClass);
				//alert($("#besth").length);
					$('#besth').children().each(function(n)
					{
						$(this).remove();
					});
					$('#besth').remove();
					$('.avgrund-overlay ' + options.overlayClass).remove();
