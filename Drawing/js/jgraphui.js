...
(function ($) {
    $.fn._t = function (nodeId, dgrm) {
        ...
        self.bind('dblclick', function () {
			show_tooltip(2);
			$("#"+nodeId).show(100);
			// 선택된 클래스와 노드로 연결된 클래스들
			//node_recall_nodeId(dgrm.getSelectedNodes());

			$("input[name=currentclass_node]").val(nodeId);
			var initWidth = $(this).width();
            var initHeight = $(this).height();
			$("input[name=var_classname]").val(self.value);
			$("#tool_properties").show();
            self.html('<input type="text" name="modify_classname" style="background: url(\'images/transparent.gif\')" value="' + self.value + '"/>').find('input').bind('blur', function (event) {
				...
				//$("#"+currentClass+"_var").html(Class.showLocalStorage("variable"));
            }).keydown(function (e) {
            ...
            }).width(initWidth).height(initHeight).css({
                'border': '0'
            }).focus()
        })
    }
	...
	/**
	*
	*	클래스 간 마우스 드래그 이벤트 간의 임시 라인
	*
	*	@param	node	출발 클래스 노드ID
	*	@param	direction 도착 클래스의 관계 방향
	*	@param	canvasobj 캔버스 타겟
	*	@param	line 선의 종류
	*	@return 없음
	*/
...
function dtc(node, direction, port, canvasobj, line) {
	...
	*
	*	@param dgrm 대상 다이어그램
	*	@param	params 툴바 출력 on/off 파라미터
	*	@return 없음
	*/
...
Node = function (params) {
	...
	if(node.nodeId) activateClass(node.nodeId, "New Class"); //이게 왜 필요한지 몰라서 주석했었다.20130114황설희
    ...
    node.height = (params["height"]) ? params["height"] : 80;
    node.bgColor = (params["bgColor"]) ? params["bgColor"] : "#FFFFFF";
    node.borderColor = (params["borderColor"]) ? params["borderColor"] : "#ffaa22";
...
Diagram = function (params) {
    ...
    this.connectionWidth = (params["connectionWidth"]) ? params["connectionWidth"] : 2;
    ...
    this.addNode = function (node) {
        ...
        container += 'class="ui-widget-content node" style="width: ' + node.width + 'px; height: ' + node.height + 'px;left:' + node.xPosition + 'px; top:' + node.yPosition + 'px ;border:0px; cursor:hand">';
        ...
        container += '</td><td id="' + node.nodeId + '_content"  valign=middle align=center style="border: 1px solid ' + node.borderColor + ';background-color: ' + node.bgColor + ';FONT-FAMILY: ' + node.fontType + '; COLOR: ' + node.fontColor + '; FONT-SIZE: ' + node.fontSize + '; FONT-WEIGHT: bold; TEXT-DECORATION: none; border-radius: 4px; background: -webkit-linear-gradient(top,#fee54c,#fd9b10 100%); background: -moz-linear-gradient(top,#fee54c,#fd9b10 100%); background: -ms-linear-gradient(top,#fee54c,#fd9b10 100%);background: -o-linear-gradient(top,#fee54c,#fd9b10 100%); background: linear-gradient(top,#fee54c,#fd9b10 100%); ';
        if (node.nodeType == 'TEXT' || node.nodeType == 'IMAGE') container += 'background: url(\'' + self.imagesPath + 'transparent.gif\') repeat;  ';
        container += '">';
        if (node.nodeType == 'IMAGE') container += '<img id="' + node.nodeId + '_imagenode" src="' + node.image + '" width=' + node.imageWidth + 'px height=' + node.imageHeight + 'px border=0>';
        ...
        if (node.nPort) {
            if (node.nPortMakeConnection) {
                $("#" + node.nodeId + "_n").draggable({
                    ...
                    stop: function () {
						callerObj.setConnectionRelation(node.nodeId,callerObj);
                        $(ports).css("visibility", "hidden");
            ...
            if (node.ePortAcceptConnection) {
                $("#" + node.nodeId + "_e").droppable({
                    ...
                    drop: function (event, ui) {
						...
						node_connect(sourceNode,targetNode);
                        callerObj.clearTempLine();
                        var connection = new Connection(sourceNode, sourceNodePort.charAt(sourceNodePort.length - 1), targetNode, targetNodePort.charAt(targetNodePort.length - 1), callerObj.connectionColor, callerObj.connectionWidth, "composition");
                        callerObj.addConnection(connection)
                    }
                })
            }
    ...
    this.deleteConnection = function (connectionId,connection) {
		$("#selectconnection").val(connectionId);
		this.reConnectionRelation(connection.nodeFrom, connectionId,connection);
    };
	/**
	*
	*	[inline] 관계 수정 함수
	*	@return 없음
	*/
	this.reConnectionRelation = function (nodeId,connId,connection){
			caller = this;
			$("#rel_del").click(function  () {
							for (i = 0; i < caller.connections.length; i++) {
								if (caller.connections[i].connectionId == $("#selectconnection").val()) {
									caller.connections[i].line.clear();
									caller.connections.splice(i, 1);
									//alert(connection.nodeTo + "," + connection.nodeFrom + "," + $("#" + connection.nodeTo + "_content").html());
									// 지환아 여기에 3개 인수를 알아주면 된다.
									deleteRelation(connection.nodeFrom, connection.nodeTo, $("#" + connection.nodeTo + "_content").html());
									break;
								}
							}
				$("#tool_relation").hide();
			...
			caller = this;
		    var view_x = $("#" + nodeId).position().left + 200;
			var view_y = $("#" + nodeId).position().top + 200;
			$("#tool_relation").css("left", view_x +"px");
			$("#tool_relation").css("top", view_y +"px");
			$("#tool_relation").show();

			$("#rel_inh").click(function  () {
				for (i = 0; i < caller.connections.length; i++) if(caller.connections[i].connectionId ==  $("#selectconnection").val()) {
					caller.connections[i].relation = "inheritance"; 
					changingRelation(nodeId, $("#" + caller.connections[i].nodeTo).text() , "inheritance")
				}
				caller.setNodeAffectedConnections(nodeId);
				caller.reconnNodeAffectedConnections(nodeId,connId);
				$("#tool_relation").hide();
				caller.toJSON();
			});
			$("#rel_dep").click(function  () {
				for (i = 0; i < caller.connections.length; i++) if(caller.connections[i].connectionId == $("#selectconnection").val()) {
					caller.connections[i].relation = "dependency";
					changingRelation(nodeId, $("#" + caller.connections[i].nodeTo).text() ,  "dependency")
				}
				caller.setNodeAffectedConnections(nodeId);
				caller.reconnNodeAffectedConnections(nodeId,connId);
				$("#tool_relation").hide();
			...
			$("#rel_ass").click(function  () {
				for (i = 0; i < caller.connections.length; i++) if(caller.connections[i].connectionId ==  $("#selectconnection").val()) {
					caller.connections[i].relation = "association";
					changingRelation(nodeId, $("#" + caller.connections[i].nodeTo).text() ,  "association")
				}
				caller.setNodeAffectedConnections(nodeId);
				caller.reconnNodeAffectedConnections(nodeId,connId);
				$("#tool_relation").hide();
			...
			$("#rel_agg").click(function  () {
				for (i = 0; i < caller.connections.length; i++) if(caller.connections[i].connectionId ==  $("#selectconnection").val()) {
					caller.connections[i].relation = "aggregation";
					changingRelation(nodeId, $("#" + caller.connections[i].nodeTo).text() ,  "aggregation")
				}
				caller.setNodeAffectedConnections(nodeId);
				caller.reconnNodeAffectedConnections(nodeId,connId);
				$("#tool_relation").hide();
			...
			$("#rel_com").click(function  () {
				for (i = 0; i < caller.connections.length; i++) if(caller.connections[i].connectionId == $("#selectconnection").val()) {
					caller.connections[i].relation = "composition";
					changingRelation(nodeId, $("#" + caller.connections[i].nodeTo).text() ,  "composition")
				}
				caller.setNodeAffectedConnections(nodeId);
				caller.reconnNodeAffectedConnections(nodeId,connId);
				$("#tool_relation").hide();
	...
	*	@return 없음
	*/
	this.setConnectionRelation = function (nodeId,callerObj){
			show_tooltip(3);
			this.toJSON();
			caller = this;
		    var view_x = $("#" + nodeId).position().left + 200;
			...
			$("#tool_relation").css("top", view_y +"px");
			$("#tool_relation").show();
			for (i = 0; i < this.connections.length; i++) if (this.connections[i].nodeFrom == nodeId || this.connections[i].nodeTo == nodeId) $("#selectconnection").val(this.connections[i].connectionId);
			...
			$("#rel_inh").click(function  () {
				...
				caller.redrawNodeAffectedConnections(nodeId);
				$("#tool_relation").hide();
				caller.toJSON();
			});
			$("#rel_dep").click(function  () {
    ...
    this.reconnNodeAffectedConnections = function (nodeId,connId) {
        var caller = this;
        var node = '';
		...
		var affectedConnectionsRelation = node.affectedConnectionsRelation;
        for (i = 0; i < affectedConnections.length; i++) {
			if(affectedConnections[i].connectionId == connId){
				//alert(affectedConnections[i].connectionId);
	...
	this.toJSON = function () {
		...
		{
		    if(i>0) json_conn += ',';
		    json_conn += '{"connectionId":"' + this.connections[i].connectionId + '","nodeFrom":"' + this.connections[i].nodeFrom + '","nodeTo":"' + this.connections[i].nodeTo + '","portFrom":"' + this.connections[i].portFrom + '","portTo":"' + this.connections[i].portTo + '","color":"' + this.connections[i].color + '","stroke":"' + this.connections[i].stroke + '","relation":"' + this.connections[i].relation + '"}';
		}
		json_conn += ']';
	...
	this.fromJSON = function () {
        if (confirm('기존에 그린 다이어그램을 불러오시겠습니까?')) {
		...
		if(load_class)
		{
			for(i = 0; i<load_class.length; i++)
			{
				...
				self.addNode(new Node({
							...
							'width': load_class[i].width,
							'height': load_class[i].height,
							'affectedConnections':[]
				}))
			}
	...
	*	라인 생성 함수
	*
	*	@sourceNode 출발 클래스 노드ID
	*	@targetNode 도착 클래스 노드ID
	*	@sourcePort 출발 클래스 포트
	*	@targetPort 도착 클래스 포트
	*	@line 선의 굵기
	*	@canvasobj 대상 캔버스
	*	@relation 관계 종류
	*	@return 없음
